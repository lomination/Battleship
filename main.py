...
class Ds:
    ...
    def update(self, sea: Board, screen: pg.Surface, in_menu: bool = False) -> None:
        """Update the display settings values"""
        self.tile_s = min(
            screen.get_size()[0] // (sea.size()[0] + (1 if in_menu else 0)),
            screen.get_size()[1] // (sea.size()[1] + (1 if in_menu else 0))
        )
        self.x_margin = (screen.get_size()[0] - (sea.size()[0] + 1) * self.tile_s) // 2,
        self.y_margin = (screen.get_size()[1] - (sea.size()[1] + 1) * self.tile_s) // 2,


def draw_board(sea: Board, screen: pg.Surface, ds: Ds, super: bool) -> None:
    """Displays the given board on the given screen
    
    Parameters
    ----------
    
    - sea: the board to display
    ...
    DARK_BLUE: tuple[int, int, int] = (0, 105, 148)
    ...
    # start drawing
    for y in range(sea.size()[1]):
        for x in range(sea.size()[0]):
            current_tile: Tile = sea.get_tile_at((x, y))
            colour: tuple[int, int, int]
            if current_tile.get_state() == State.NOTSEEN and not super:
                colour = GREY
            elif current_tile.get_boat_id() is None:
                colour = DARK_BLUE
            elif super or sea.get_boats()[current_tile.get_boat_id()] == State.SEEN:
                colour = boat_colour(current_tile.get_boat_id(), sea.get_boats())
            else:
                colour = WHITE
...
def main() -> None:
    ...
    try:
        ...
        while not menu_end:
            for event in pg.event.get():
                if event.type == pg.MOUSEBUTTONDOWN:
                    ...
                    if ds.x_m <= mouse_x <= screen.get_size()[0] - ds.x_m and ds.y_m <= mouse_y <= screen.get_size()[1] - ds.y_m:
                        ...
                        elif (screen.get_size()[0] - ds.tile_s) // 2 <= mouse_x <= screen.get_size()[0] - ds.x_m - ds.tile_s and screen.get_size()[1] - ds.y_m - ds.tile_s <= mouse_y <= screen.get_size()[1] - ds.y_m and sea.size()[1] > 1:
                        ...
                        elif screen.get_size()[0] - ds.x_m - ds.tile_s <= mouse_x <= screen.get_size()[0] - ds.x_m and (screen.get_size()[1] - ds.tile_s) // 2 <= mouse_y <= screen.get_size()[1] - ds.y_m - ds.tile_s and sea.size()[0] > 1:
                            # delete a column to the board
                            sea.del_column()
                            # update size of tiles and margins' width
                            ds.update(sea, screen, True)
                        # if the mouse is on the start button and at least one boat has been placedthen
                        elif screen.get_size()[0] - ds.x_m - ds.tile_s <= mouse_x <= screen.get_size()[0] - ds.x_m and screen.get_size()[1] - ds.y_m - ds.tile_s <= mouse_y <= screen.get_size()[1] - ds.y_m and len(sea.get_boats().keys()) > 0:
                            # end the menu
                            menu_end: bool = True
                            # start the game
                            game_end: bool = False
                        # else the mouse is on the grid then
                            ...
                            current_tile: Tile = sea.get_tile_at(((mouse_x - ds.x_m) // ds.tile_s, (mouse_y - ds.y_m) // ds.tile_s))
                            if current_tile.get_boat_id() is not None:
                                # delete the selected boat
                                sea.del_boat(current_tile.get_boat_id())
                            else:
                                # prepare to add a new boat
                                boat_start_x = (mouse_x - ds.x_m) // ds.tile_s
                                boat_start_y = (mouse_y - ds.y_m) // ds.tile_s
                    ...
                    if ds.x_m <= mouse_x < screen.get_size()[0] - ds.x_m - ds.tile_s and ds.y_m <= mouse_y < screen.get_size()[1] - ds.y_m - ds.tile_s and boat_start_x != None and boat_start_y != None:
                        ...
                        if len(sea.get_boats().keys()) == 0:
                            new_boat_id: int = 0
                        else:
                            new_boat_id: int = sorted(sea.get_boats().keys())[-1] + 1
                        ...
                        if abs(boat_end_x - boat_start_x) >= abs(boat_end_y - boat_start_y):
                            if boat_end_x - boat_start_x > 0:
                                for i in range(abs(boat_end_x - boat_start_x) + 1):
                                    ...
                                    if sea.get_tile_at(position).get_boat_id() is not None:
                                        sea.del_boat(sea.get_tile_at(position).get_boat_id())
                                    sea.set_tile_at(Tile(new_boat_id, State.NOTSEEN), position)
                            else:
                                for i in range(abs(boat_end_x - boat_start_x) + 1):
                                    position: tuple[int, int] = (boat_start_x - i, boat_start_y)
                                    if sea.get_tile_at(position).get_boat_id() is not None:
            ...
            # del row button
            colour: tuple[int, int, int] = RED if sea.size()[1] > 1 else GREY
            pg.draw.rect(screen, colour, (
            ...
            # del column button
            colour: tuple[int, int, int] = RED if sea.size()[0] > 1 else GREY
            ...
            # start button
            colour: tuple[int, int, int] = RED if len(sea.get_boats().keys()) > 0 else GREY
            ...
            # draw board
            draw_board(sea, screen, ds, True)
            pg.display.flip()
        ds.update(sea, screen, False)
        # draw background
        screen.fill(BLACK)
        draw_board(sea, screen, ds, False)
        pg.display.flip()
        # ---------- GAME ---------- #
        while not game_end:
            for event in pg.event.get():
                if event.type == pg.MOUSEBUTTONDOWN:
                    mouse_x, mouse_y = pg.mouse.get_pos()
                    if ds.x_m <= mouse_x < screen.get_size()[0] - ds.x_m and ds.y_m <= mouse_y < screen.get_size()[1] - ds.y_m:
                        sea.guess_tile(((mouse_x - ds.x_m) // ds.tile_s, (mouse_y - ds.y_m) // ds.tile_s))
                        if sea.is_finished():
                            # pg.draw.rect(screen, (0, 0, 0), (
                            #     ds.x_m - ds.half_tile_m,
                            #     ds.y_m - ds.half_tile_m,
                            #     sea.size()[0] * ds.tile_s + 2 * ds.half_tile_m,
                            #     sea.size()[1] * ds.tile_s + 2 * ds.half_tile_m
                            # ))
                            # draw background
                            screen.fill(GREEN)
                            # draw board
            ...
            screen.fill(BLACK)
            # draw board
            draw_board(sea, screen, ds, pg.key.get_pressed()[pg.K_SPACE])
            # refresh screen
            pg.display.flip()
    finally:
        pg.quit()
